...
const (
	...
	serviceAccountRequestUrl = "/api/v1/serviceaccounts"
	secretRequestUrl = "/api/v1/secrets"
)
...
func (r *RHMIReconciler) createSilence(installation *rhmiv1alpha1.RHMI, ctx context.Context, rc *rest.Config) error {
	...
	for _, alert := range alertsToSilence {
		exists, err := r.silenceExists(installation, openshiftMonitoringNamespace, openshiftAlertManagerRoute, endpoint, log, alert, ctx, rc)
		if err != nil {
			log.Error("Error checking for silence", err)
		}
		if !exists {
			//silenceAlert(openshiftMonitoringNamespace, openshiftAlertManagerRoute, endpoint, log, alert, ctx, rc)
		}
...
func (r *RHMIReconciler) silenceExists(installation *rhmiv1alpha1.RHMI, namespace string, route string, endpoint string, log l.Logger, alertName string, ctx context.Context, rc *rest.Config) (bool, error) {
	//silenceExists := false

	url, err := r.getURLFromRoute(route, namespace, ctx, rc)
	if err != nil {
		return false, err
	}
	...
	token, err := getBearerToken(installation, ctx, rc)
	if err != nil {
		//return false, err
	}
	log.Info(token)
...
func getBearerToken(installation *rhmiv1alpha1.RHMI, ctx context.Context, rc *rest.Config) (string, error) {
	...
	request := client.RESTClient().Get().Resource("").Name(serviceAccount.Name).Namespace(serviceAccount.Namespace).RequestURI(serviceAccountRequestUrl).Do(context.TODO())
	requestBody, err := request.Raw()

	log.Info(string(requestBody))

	err = json.Unmarshal(requestBody,serviceAccount)
	if err != nil {
	...
	request = client.RESTClient().Get().Resource("secrets").Name(secret.Namespace).Namespace(secret.Namespace).RequestURI("").Do(context.TODO())
	requestBody, err = request.Raw()
	err = json.Unmarshal(requestBody,secret)
...
func (r *RHMIReconciler) getURLFromRoute(routeName string, namespace string, ctx context.Context, rc *rest.Config) (string, error) {
	client, err := appsv1Client.NewForConfig(rc)
	if err != nil {
		log.Error("Error :",err)
	}
